...
function baiduApiError(json, pushSite, status) {
  ...
  if (text === 'over quota') {
    text = 'over quota：今日主动推送配额已用完，请次日再试或减少单次抓取条数';
  }
...
export async function baiduSubmitUrlsBatched({ site, token, urls, type = 'normal' }) {
  ...
  const batches = [];
  for (let i = 0; i < list.length; i += BAIDU_BATCH_SIZE) {
    batches.push(list.slice(i, i + BAIDU_BATCH_SIZE));
  }

  const results = [];
  let totalSuccess = 0;
  for (let i = 0; i < batches.length; i += 1) {
    const result = await baiduSubmitUrls({
      site,
      token,
      urls: batches[i],
      type,
    });
    results.push({ batch: i + 1, count: batches[i].length, result });
  ...
  return {
    ...
    totalSuccess,
    batches: results,
    last: results[results.length - 1]?.result,
  };
}
