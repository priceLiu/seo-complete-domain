...
export function canonicalizeUrlsForBaiduPush(urls, pushSite) {
  ...
  const siteRoot = targetHost.startsWith('www.')
    ? `https://${targetHost}`
    : `https://${targetHost}`;
  return canonicalizeUrlsToSiteHost(urls, siteRoot);
}
...
export function resolveBaiduSiteForSitemap(sitemapUrl, credentialSite) {
  ...
  // 优先凭据/验证站（多为 www），避免 sitemap 裸域导致 site= 与推送 URL 不一致
  const cred = normalizeBaiduPushSite(credentialSite);
  if (cred) return cred;
  return siteFromSitemapUrl(sitemapUrl) || '';
}
...
export async function baiduPushFromSitemap({
  ...
  const urls = canonicalizeUrlsForBaiduPush(
    canonicalizeUrlsToSiteHost(rawUrls, getSiteUrl() || `https://${pushSite}`),
    pushSite,
  );
...
export async function baiduPingSitemap({ site, token, sitemapUrl }) {
  ...
  for (const buildUrl of attempts) {
    try {
      ...
      const snippet = text.trim().slice(0, 200) || '(无响应正文)';
      lastErr = new Error(
        `Sitemap Ping 失败（ping.baidu.com，HTTP ${res.status}）：${snippet}。与 URL 主动推送配额无关，可关闭 Ping 或稍后在站长平台手动提交 Sitemap。`,
      );
    } catch (e) {
      lastErr = e;
    }
