 ...
 */
export async function syncQueueFromSitemap({ siteId, engine, sitemapUrl, maxUrls = 5000 }) {
  const urls = await fetchUrlsFromSitemapUrl(sitemapUrl, maxUrls);
  const existing = readQueue(siteId, engine);
  const doneMap = new Map();
  if (existing?.items) {
    for (const it of existing.items) {
      if (it.status === 'done') doneMap.set(it.url, it);
    }
  }
...
export async function startNextCycle({ siteId, engine, sitemapUrl, maxUrls }) {
  await syncQueueFromSitemap({ siteId, engine, sitemapUrl, maxUrls });
  const state = readQueue(siteId, engine);
