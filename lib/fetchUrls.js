...
/** 从 sitemap XML 提取 loc（避免 cheerio→undici 依赖 Node File API，SCF Node18 无 File） */
function extractLocUrls(xml) {
  ...
  while ((m = re.exec(text)) !== null) {
    const loc = m[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/i, '$1').trim();
...
export async function fetchUrlsFromSitemapUrl(sitemapUrl, maxPages) {
  ...
  async function loadOne(url) {
    ...
    const nested = [];
    for (const loc of extractLocUrls(res.data)) {
